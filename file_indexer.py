...
def generate_index(path=None, max=500):
    ...
    path = path if path else os.getcwd() # Request path if not provided
    ...
    return df
def indexDirectory(inputPath=os.getcwd(), excelPath = "file_index.xlsx"):
  df = generate_index(path=inputPath,max=0)
